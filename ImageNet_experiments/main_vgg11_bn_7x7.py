...
from vgg7x7 import vgg11_bn_7x7 
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        if i%1000 == 0 or i==10 or i==20 or i==50:
            try:
                print(torch.cuda.memory_summary(0))
                print(torch.cuda.memory_summary(1))
                print(torch.cuda.memory_summary(2))
...
def save_checkpoint(state, is_best, filename='checkpoint_7x7_vgg11_bn.pth.tar'):
    bulk_dir='save_dir'
    torch.save(state, os.path.join(bulk_dir,filename))
    if is_best:
        shutil.copyfile(os.path.join(bulk_dir,filename), os.path.join(bulk_dir,'model_best_7x7_vgg11_bn.pth.tar'))
