...
from windowed_vgg import vgg11_bn_window
...
class HybridValPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop,
                 size, shard_id, num_shards, dali_cpu=True):
...
def save_checkpoint(state, is_best, filename='checkpoint_window_vgg11_bn.pth.tar'):
    bulk_dir='save_dir'
    torch.save(state, os.path.join(bulk_dir,filename))
    if is_best:
        shutil.copyfile(os.path.join(bulk_dir,filename), os.path.join(bulk_dir,'model_best_window_vgg11_bn.pth.tar'))
